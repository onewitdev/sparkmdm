...
object MFindMatchPairs {
  ...
  def matchpairs (spark : SparkSession,appId : String,config: MDMConfig) = {
      ...
      //due to the indexedLable where label 1 got index 0, here prediction 0 means matched!!!
      //we need to keep using StringIndexed in BCalcThreshold but find a way to know idex(0)'s value is 1
      //from either the indexer or it's trained model
      var mpSQL = s"""
        SELECT distinct mid1, mid2 
        FROM tblBinClassified$tblsuffixI s
        WHERE prediction=0
      """
