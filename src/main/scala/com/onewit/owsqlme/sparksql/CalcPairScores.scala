...
object CalcPairScores {
   ...
   def genscores (spark : SparkSession,appId : String,config: MDMConfig) = {
      ...
      with attrscores as (
        ...
        mid1, 
        mid2,
        ngcompare(p.npi1, p.npi2)* avg2d(w1_1.Weight,w1_2.Weight) as sc_npi,
        ngcompare(p.plname1, p.plname2)* avg2d(w2_1.Weight,w2_2.Weight) as sc_ln,
        ngcompare(p.pfname1, p.pfname2)* avg2d(w3_1.Weight,w3_2.Weight) as sc_fn
        from       
        tblAllPairAttrs$tblsuffixItmp p --tmptblPairAttrs$appId p
      	left outer join 
              tblValueFreq$tblsuffixI w1_1 on p.npi1 = w1_1.Value and w1_1.ValType=0
      	left outer join
              tblValueFreq$tblsuffixI w1_2 on p.npi2 = w1_2.Value and w1_2.ValType=0
      	left outer join
              tblValueFreq$tblsuffixI w2_1 on p.plname1 = w2_1.Value and w2_1.ValType=1
      	left outer join
              tblValueFreq$tblsuffixI w2_2 on p.plname2 = w2_2.Value and w2_2.ValType=1
      	left outer join
              tblValueFreq$tblsuffixI w3_1 on p.pfname1 = w3_1.Value and w3_1.ValType=2
        left outer join
      	      tblValueFreq$tblsuffixI w3_2 on p.pfname2 = w3_2.Value and w3_2.ValType=2
      )
      select mid1,mid2,sc_npi , sc_ln , sc_fn, (sc_npi + sc_ln + sc_fn) as score
      from attrscores where (sc_npi + sc_ln + sc_fn)>0
      DISTRIBUTE BY mid2
      """
